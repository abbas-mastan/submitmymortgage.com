    ...
    <table class="w-full">
        ...
        <tbody>
            ...
            @foreach (config('smm.file_category') as  $cat)
                ...
                && $cat === "Purchase Agreement") ||  $cat === "Credit Report")
                    @continue
                @endif
                <tr>
                    ...
                    <td  class=" pl-2 tracking-wide border border-r-0">
                        @if (fileCatCount($cat, $user->id) > 0)
                            <form method="POST" action="{{ url(getUserRoutePrefix().'/update-cat-comments') }}" class="">
                                @csrf
                                <div class="flex space-x-4">
                                    ...
                                    <div class=" my-0.5">
                                        <button title="Add comments"  type="submit" class="bg-gradient-to-b from-gradientStart to-gradientEnd capitalize rounded-md px-3 py-0.5  focus:outline-none focus:border-none  focus:ring-1 focus:ring-blue-400 tas[i]-white">
                                            Save
                                        </button>
                                    </div>
                                </div>
