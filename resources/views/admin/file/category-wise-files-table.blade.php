        ...
        @if ($cat === "Credit Report")
            ...
            <tr>
                ...
                <td class=" pl-2 tracking-wide border border-l-0">
                    <a href="{{ url(getAdminRoutePrefix().'/docs/'.$user->id.'/'.str_replace('/','-',$cat)) }}" class="hover:text-blue-700">
                        {{ $cat }}
                    </a>
                ...
                <td class=" pl-2 tracking-wide border capitalize">
                    <div class=" flex justify-center">
                        ...
                        @elseif (!fileCatSubmittedByClient($cat, $user->id))
                ...
                <td class=" pl-2 tracking-wide border capitalize">
                    <div class="  flex flex-col xxl:flex-row justify-center items-center xxl:space-x-4 space-y-2 xxl:space-y-0">
                        ...
                        @elseif (!fileCatVerified($cat, $user->id))
                            ...
                            @if (fileCatCount($cat, $user->id) > 0)
                                ...
                                <form method="POST" action="{{ url(getAdminRoutePrefix().'/update-category-status') }}" class="">
                                    @csrf
                ...
                <td  class=" pl-2 tracking-wide border border-r-0">
                    @if (fileCatCount($cat, $user->id) > 0)
                        <form id="status-form" method="POST" action="{{ url(getAdminRoutePrefix().'/update-cat-comments/'.str_replace('/','-', $cat)) }}" class="">
                            @csrf
                            <div class="flex space-x-4">
                                ...
                                <div class=" my-0.5">
                                    <button title="Save category comments"  type="submit" class="bg-gradient-to-b from-gradientStart to-gradientEnd capitalize rounded-md px-3 py-0.5  focus:outline-none focus:border-none  focus:ring-1 focus:ring-blue-400 text-white">
                                        Save
                                    </button>
                                </div>
                            </div>
