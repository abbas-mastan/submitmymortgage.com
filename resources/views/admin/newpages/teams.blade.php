...
@section('content')
    ...
    @if (count($enableTeams) > 0)
        <h2 class="enableTeamsHeader text-center text-xl border-y-4 py-3  mt-5">Enabled Teams</h2>
    @endif
    @foreach ($enableTeams as $team)
        @if (!$team->disable)
            ...
            @component('components.accordion', [
                'title' => $team->name . ' | ' . $team->company->name,
                'created_by' => $team->owner->name ?? null,
            ])
                ...
                @if ($team->owner_id == Auth::id() || $currentrole === $superadminrole || $currentrole === 'Admin')
                    <div class="flex justify-between mt-5">
                        ...
                        <a>
                            <button class="addMembers bg-red-800 px-5 py-2 text-white" company="{{ $team->company_id }}"
    ...
    @if (isset($disableTeams) && count($disableTeams) > 0)
        <h2 class="disableTeamsHeader text-center text-xl border-y-4 py-3  mt-5">Disabled Teams</h2>
    @endif
    @isset($disableTeams)
        @foreach ($disableTeams as $key => $team)
            @if ($team->disable)
                <input class="disableTeams hidden" value="{{ $loop->iteration }}">
                @component('components.accordion', ['title' => $team->name, 'created_by' => $team->owner->name ?? null])
                    <table class="w-full display shadow-lg" id="{{ Str::slug($team->name) }}-table{{ $team->id }}">
                        <thead class="hidden bg-gray-300">
                            <tr>
                                <th class="pl-2 tracking-wide">
