<tr>
    ...
    <td class="text-center pl-2 tracking-wide border border-r-0">
        @if ($key !== 3)
            <a class="delete" data="{{ $currentrole === $superadminrole ? 'temporary' : 'Delete' }}"
                title="Click to Delete this application"
        ...
        @endif
        @if ($key == 0)
            <a class="delete mx-2" data="Accept" title="Click to Accept this application"
                href="{{ url(getRoutePrefix() . '/application-update-status/' . $application->id . '/accept') }}">
                <button class="bg-black tracking-wide font-semibold capitalize text-xl">
                    <img src="{{ asset('icons/tick.svg') }}" alt="" class="p-1 w-7">
                </button>
            </a>
            <a class="delete" data="Reject" title="Click to Reject this application"
        ...
        @endif
        @if ($key === 3 && isSuperAdmin())
            <a class="delete mx-2" data="Restore" title="Click to restore this application"
